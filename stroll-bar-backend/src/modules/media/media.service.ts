...
export class MediaService {
	...
	async initiateMultipartUpload(dto: CreatePresignedUploadDto, userId: string) {
		...
		if (dto.sizeBytes < this.multipartThresholdBytes) {
			throw new BadRequestException(`Use the single-part upload endpoint for files smaller than ${this.multipartThresholdBytes} bytes.`);
		}

		const partCount = Math.ceil(dto.sizeBytes / this.multipartPartSizeBytes);

		if (partCount > this.multipartMaxParts) {
			throw new BadRequestException(`Multipart uploads may not exceed ${this.multipartMaxParts} parts with the current part size.`);
		}
	...
	private assertUploadPolicy(contentType: string, sizeBytes: number): void {
		if (contentType.startsWith('image/')) {
			...
			if (sizeBytes > this.maxImageSizeBytes) {
				throw new BadRequestException(`Image uploads may not exceed ${this.maxImageSizeBytes} bytes.`);
			}
		...
		if (contentType.startsWith('video/')) {
			...
			if (sizeBytes > this.maxVideoSizeBytes) {
				throw new BadRequestException(`Video uploads may not exceed ${this.maxVideoSizeBytes} bytes.`);
			}
